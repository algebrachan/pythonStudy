...
def batch_add_fur():
    ...
    for i in series_list:
        for j in range(1, 97):
            ...
            r = requests.post('http://10.50.60.206:8065/api/mvt/update_fur_state',
...
def batch_add_history_broken():
        ...
        for j in get_data_list(30):
            ...
            r = requests.post(
                'http://10.50.60.206:8065/api/mvt/update_history_broken', data=json.dumps(body))
            print(r.text)
...
def interv_add_history_broken():
  ...
  r = requests.post('http://10.50.60.206:8065/api/mvt/update_history_broken', data=json.dumps(body))
  print(r.text)
...
def batch_add_stic_data():
    for furnace_id in ['2B20','2B21','2B22','2B23']:
        ...
        r_temp = requests.post('http://10.50.60.206:8065/api/anls/update_temp_process', data=json.dumps(temp_body))
        print(r_temp.text)
        seed_body = {
            "furnace_id":furnace_id,
        ...
        r_seed = requests.post('http://10.50.60.206:8065/api/anls/update_seed_process', data=json.dumps(seed_body))
        print(r_seed.text)
        shoulder_body = {
            "furnace_id":furnace_id,
        ...
        r_shoulder = requests.post('http://10.50.60.206:8065/api/anls/update_shoulder_process', data=json.dumps(shoulder_body))
        ...
        r_cyl = requests.post('http://10.50.60.206:8065/api/anls/update_cyl_process', data=json.dumps(cyl_body))
        print(r_cyl.text)
