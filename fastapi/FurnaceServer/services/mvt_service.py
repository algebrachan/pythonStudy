...
async def fur_state(item: ReqUpdateFurState, res: ResponseBase, host: str):
    session = mysql_session.session()
    try:
        data = await session.query(FurnaceList).filter(
...
def current_fur_list(series: str, res: ResponseBase):
    session = mysql_session.session()
    data = RespCurrentFurList()
    r_key = data.get_rname()+series
    try:
        if redis_session.exists(r_key):
            data = redis_session.get_redis_str(r_key)
            pass
            ...
            if len(db_data) > 0:
                for i in db_data:
                    data.fur_list.append(CurrentFurList(
                        series+str(i[0]), i[1]).__dict__)
                data.series = series
                redis_session.set_redis_str(data.__dict__, r_key, 30)
                pass
            else:
...
def server_state(server_ip: str, res: ResponseBase):
    ...
    try:
        if redis_session.exists(r_key):
            ...
            redis_session.set_redis_str(data.__dict__, r_key, 31)
            pass
        res.data = data
    except Exception as e:
        res.error(Error.SERVER_EXCEPTION)
...
def online_fur(res: ResponseBase):
    ...
    try:
        ...
        if redis_session.exists(r_key):
            ...
            data.online_list = mysql_init_data(db_data,dict_model,CommonListItem)
            if len(data.online_list) > 0:
...
def history_broken(type: int, res: ResponseBase):
    session = mysql_session.session()
    data = RespHistoryBroken()
    try:
        r_key = data.get_rname()+str(type)
        if redis_session.exists(r_key):
            data = redis_session.get_redis_str(r_key)
            pass
        else:
            ...
            if type == 1:  # shouldering
                ...
                pass
            for i in db_data:
                data.broken_list.append(CommonListItem(
                    str(i[0]), int(i[1])).__dict__)
            if len(data.broken_list) > 0:
                redis_session.set_redis_str(data.__dict__, r_key, 60*60*2)
            pass
    except Exception as e:
        res.error(Error.SERVER_EXCEPTION)
        logger.error(e)
